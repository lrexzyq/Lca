...
Library.AccentColor = Library.Scheme and Library.Scheme.AccentColor or Color3.fromRGB(0, 200, 255)
Library.MainColor = Library.Scheme and Library.Scheme.MainColor or Color3.fromRGB(25, 25, 25)
Library.BackgroundColor = Library.Scheme and Library.Scheme.BackgroundColor or Color3.fromRGB(15, 15, 15)
Library.OutlineColor = Library.Scheme and Library.Scheme.OutlineColor or Color3.fromRGB(40, 40, 40)
Library.FontColor = Library.Scheme and Library.Scheme.FontColor or Color3.new(1, 1, 1)
Library.Font = Library.Scheme and Library.Scheme.Font or Font.fromEnum(Enum.Font.Code)

if not Library.Create then
    function Library:Create(className, properties)
        local obj = Instance.new(className)
        for key, value in pairs(properties or {}) do
            if key == "Font" and typeof(value) == "EnumItem" then obj.Font = value
            elseif key == "Font" and typeof(value) == "Font" then obj.FontFace = value
            else obj[key] = value end
        end
        return obj
    end
end
if not Library.CreateLabel then
    function Library:CreateLabel(properties) return self:Create("TextLabel", properties or {}) end
end
if not Library.AddToRegistry then function Library:AddToRegistry() end end
if not Library.UpdateColorsUsingRegistry then function Library:UpdateColorsUsingRegistry() end end
if not Library.GetDarkerColor then
    function Library:GetDarkerColor(color)
        local h,s,v=Color3.toHSV(color)
        return Color3.fromHSV(h,s,math.clamp(v*0.65,0,1))
    end
end
if not Library.SetWatermarkVisibility then
    function Library:SetWatermarkVisibility(visible)
        if self.Watermark then self.Watermark.Visible=visible end
    end
end
if Library.Scheme then Library.Scheme.AccentColor=Color3.fromRGB(0,200,255) end
...
if UIS.TouchEnabled then
    ...
    local function addMobileToggle(text, default, callback)
        ...
        if tab then
            local group = tab:AddLeftGroupbox("Mobile")
            if group then
                group:AddToggle(text, {
                    Text = text,
    ...
    menuBtn.Position = UDim2.new(1, -70, 0, 15)
    ...
    lockBtn.Position = UDim2.new(1, -65, 0, 80)
    ...
    lockBtn.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.Touch then
            locked = not locked
            if Window and Window.Holder then
                Window.Holder.Active = not locked
                Window.Holder.Draggable = not locked
                Window.Holder.Selectable = not locked
            end
            lockBtn.ImageColor3 = locked and Color3.fromRGB(255, 50, 50) or Color3.fromRGB(255, 255, 255)
        end
    end)

    task.defer(function()
        local vp = workspace.CurrentCamera.ViewportSize
        local w = math.min(vp.X * 0.88, 650)
        local h = math.min(vp.Y * 0.82, 500)
        if Window and Window.Holder then
        ...
        if Library and Library.ScreenGui then
            for _, v in pairs(Library.ScreenGui:GetDescendants()) do
                if v:IsA("Frame") and v.Size and v.Size.X.Offset > 0 then
                    local scaleX = math.min(1, w / 700)
                    local scaleY = math.min(1, h / 600)
